...
def load_all_data():
    ...
    files = ['stones_main_data.json', 'lookup_groups.json', 'lookup_days.json', 
             ...
             'lookup_chakra.json', 'lookup_elements.json', 'lookup_numerology.json']
    try:
        for f in files:
        ...
        messagebox.showerror("Data Error", f"Failed to load JSON data: {e}")
        return None
    
    if not data.get('stones'):
        messagebox.showinfo("Data Load", "Cannot run without stones_main_data.json. Please ensure data files are present.")
        return None
    return data
...
class PyStoneApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.search_frame = ttk.LabelFrame(main_frame, text="⚙️ เครื่องมือค้นหาและจัดการ", padding="10")
        ...
        for text, mode in modes:
            btn = ttk.Button(tab_frame, text=text, 
                             command=lambda m=mode: self.switch_search_mode(m))
        ...
        self.top_pagination_frame.pack(side='right', padx=10)
        ...
        self.top_summary_label = ttk.Label(self.summary_control_frame, text="ข้อมูลสรุปการค้นหา", foreground='blue', justify='left')
        self.top_summary_label.pack(side='left', padx=10, fill='x', expand=True)
        ...
        self.tree.heading('Chakra', text='จักระ', anchor='center'); self.tree.column('Chakra', width=100, anchor='w')
        self.tree.heading('Element', text='ธาตุ', anchor='center'); self.tree.column('Element', width=100, anchor='w')
        self.tree.heading('Numerology', text='เลขมงคล', anchor='center'); self.tree.column('Numerology', width=100, anchor='w')
    ...
    def setup_pagination_controls(self, parent_frame: ttk.Frame, is_top: bool):
        ...
        getattr(self, f'page_info_label_{label_text}').pack(side='right', padx=10)
        ...
        if is_top:
            ttk.Button(parent_frame, 
                       ...
                       style='AddButton.TButton').pack(side='right', padx=10)
    ...
    def create_search_mode_frame(self, mode: str) -> ttk.Frame:
        ...
        if mode == 'name':
            ...
            ttk.Label(frame, text="วัน/เดือน/ปีเกิด (DD/MM/YYYY พ.ศ.):").pack(side='left', padx=5, pady=5)
            self.date_search_entry = ttk.Entry(frame, width=20)
            self.date_search_entry.pack(side='left', padx=5, pady=5)
            # FIX: ใช้ Style ปุ่มค้นหาใหม่
            ttk.Button(frame, text="ค้นหาหินตาม วดป.", command=lambda: self.filter_data(mode), style='SearchButton.TButton').pack(side='left', padx=10)
    ...
    def filter_data(self, mode: str):
        ...
        try:
            if mode == 'name':
                ...
                if not date_th:
                    messagebox.showwarning("Warning", "กรุณาระบุ วัน/เดือน/ปีเกิด")
                    return
    ...
    def show_action_popup(self, stone, event):
        ...
        ttk.Button(button_frame, text="G. ค้น Google", command=lambda: [self.search_external(stone, 'Google'), popup.destroy()]).pack(fill='x', padx=20, pady=2)
        ttk.Button(button_frame, text="E. ค้น Etsy", command=lambda: [self.search_external(stone, 'Etsy'), popup.destroy()]).pack(fill='x', padx=20, pady=2)
